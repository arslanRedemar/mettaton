...
module.exports = {
  ...
  async execute(interaction, repository) {
    ...
    if (!lecture.hasAttendee(interaction.user.id)) {
      return interaction.reply('⚠️ 당신은 이 강의에 참석 등록되어 있지 않습니다.');
    }

    lecture.removeAttendee(interaction.user.id);
    ...
    await interaction.reply(`✅ 강의 **[${lecture.title}]** 참석이 취소되었습니다.`);
  },
};
